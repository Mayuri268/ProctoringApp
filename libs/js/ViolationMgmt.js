...
function ViolationMgmt(data)
{
   ...
   $.each(data, function (key, row) {
      $('#AgentsData').append('<tr><td>' + row.violation_type + '</td><td>' + row.user_name + '</td>' +
         ...
         '<input class="custom-control-input" id="FP_' + row.user_id + '" type="radio" name="markas" onclick=radio_func("FP_' + row._id + '_' + row.user_name + '")>' +
         '<span class="custom-control-label">FP</span></label>' +
         '<label class="custom-control custom-radio custom-control-inline">' +
         '<input class="custom-control-input" id="ES_' + row.user_id + '" type="radio" name="markas" onclick=radio_func("ES_' + row._id + '_' + row.user_name + '")>' +
         '<span class="custom-control-label">ES</span></label>' +
         '<label class="custom-control custom-radio custom-control-inline">' +
         '<input class="custom-control-input" id="NA_' + row.user_id + '" type="radio" name="markas" onclick=radio_func("NA_' + row._id + '_' + row.user_name + '")>' +
         '<span class="custom-control-label">NA</span></label>' +
         '<label class="custom-control custom-radio custom-control-inline">' +
         '<input class="custom-control-input" id="VI_' + row.user_id + '" type="radio" name="markas" onclick=radio_func("VI_' + row._id + '_' + row.user_name + '")>' +
       ...
       success: function (data) {
         ...
         if(violationData.length > 0 || violationData.total_rows > 0)
         {
           $.jqPaginator('#pagination1', {
            ...
            visiblePages: Math.ceil(violationData.total_rows/10),
            currentPage: 1,
            prev: '<li class="page-item "><span class="page-link"><a href="javascript:;">Previous</a></span></li>',
            next: '<li class="page-item"><span class="page-link"><a href="javascript:;">Next</a></span></li>',
            page: '<li class="page-item" aria-current="page" id="page{{page}}"><span class="page-link"><a href="javascript:;">{{page}}</a></span></li>',

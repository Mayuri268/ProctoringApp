...
def insert_document(func_name,error):
    ...
    cluster = MongoClient(mongo_url)
    db = cluster["ProctoringAPI_DB"]
    collection = db["Logsdata"]
    data = {"function_name":func_name,"error_msg":error,"error_timing":datetime.datetime.now()}
...
def ProjectListData(PageNo):
    try:
        ...
        logfile = open('log.txt', 'w')
        logfile.write("ProjectListData Error"+str(ex)) 
        logfile.close() 
        insert_document("ProjectListData",str(ex))  
